...
# 画像データのパス(list)を指定
img_list = ['test1.jpg', 'test2.jpg', 'test3final.png', 'test5.png']
...
def preprocess_image(img):
    ...
    # 閾値処理で眼底領域（黒以外の領域）を特定
    # R, G, Bのいずれかが閾値(例えば10)を超えている画素を有効とする
    mask = img_np > 10
    if mask.ndim == 3:
    ...
    # 視神経乳頭の位置による左右判定と反転
    # 検出用にリサイズ
    img_small = img.resize((384, 384), Image.BICUBIC)
    ...
    if coords.size > 0:
        ...
        if mean_x < width / 2:
            ...
            # print("  [INFO] Left eye detected. Flipped horizontally.")
